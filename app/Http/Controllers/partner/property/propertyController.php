...
namespace App\Http\Controllers\partner\property;
...
class propertyController extends Controller
{
   ...
   function all_properties(){
    $all_properties= mak_properties::orderBy('created_at')->where('partner_id',Auth::user()->id)->where('status',1)->get()->all();
    foreach ($all_properties as $inst)
    {
       ...
       return view('partnerdashboard.properties.all_property',compact('all_properties'));
   }
   ...
   function add_pgpost(Request $request){
       ...
       if ($files = $request->file('images')) {
         ...
         return redirect()->route('partnerallproperties');
...
{
    $checkforuser=mak_properties::checkuser($id,auth::user()->id);
    if (!is_null($checkforuser)) {
        ...
        return view('partnerdashboard.properties.edit_property',compact('property_data','latestmapapi','all_state'));
    }
    alert()->error('This Property Belogs to the other Partner.', 'Property Not Belogs to you.');
    return back();    
...
{
 $all_properties_item = property_item::orderBy('created_at', 'DESC')->where('partner_id',Auth::user()->id)->where('status', 1)->get();
   if ($all_properties_item) {
    ...
    return  view('partnerdashboard.properties.property_item.index',compact('all_properties_item'));
}
public function createpropertyitem(){
    $all_partners= partner::orderBy('created_at')->where('status',1)->get()->all();
    $all_features= property_features::orderBy('created_at')->where('status',1)->get()->all();
    return view('partnerdashboard.properties.property_item.add_item',compact('all_partners','all_features'));
}
    ...
    $randomurl = $this->random_strings(30); 
        ...
        if ($files = $request->file('images')) {
                foreach($request->file('images') as $img) {   
                    ...
                    $insert = mak_propert_images::create(['property_id' =>$property_id,'img_name'=> $filePath,'alt_name' =>$name]);
                    alert()->success('You Data has been saved Prperly.', 'Save Sucessfully');
                    return redirect()->route('partnerpropertyitemindex');}
                }else{
                    alert()->success('You Data has been saved Prperly.', 'Save Sucessfully');
                    return redirect()->route('partnerpropertyitemindex');           
        }       
...
{
    $checkforuser=property_item::checkuser($id,auth::user()->id);
    if (!is_null($checkforuser)) {
...
public function edititem($id){
    ...
    if (!is_null($checkforuser)) {
        ...
        $all_partners= partner::orderBy('created_at')->where('status',1)->get()->all();
        $all_features= property_features::orderBy('created_at')->where('status',1)->get()->all();
        return view('partnerdashboard.properties.property_item.edit_item',compact('property_data','all_partners','all_features'));
    }
    alert()->error('This Property Belogs to the other Partner.', 'Property Not Belogs to you.');
        return back();  
}
